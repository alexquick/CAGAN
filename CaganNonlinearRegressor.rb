...
class CaganNonlinearRegressor
  ...
  # CaganTable table -- table of data
  ...
  #          }
  #
  # Note: MODIFIES current_estimators in place ... this is our actual answer
  def estimate (table, lhs, current_estimators, &f)
    ...
    table.rows.times do |row|
      context = CaganRegressionContext.new(row, table, best[:estimators])
      got = f.call(context);
  ...
  def run(table, lhs, estimator_hash, &f)
    error = {:r => 0, :r_sq => 0}
    table.rows.times do |row|
      context = CaganRegressionContext.new(row, table, estimator_hash)
      got = f.call(context)
      expected = table[row][lhs]
  ...
  def next_estimate(table, lhs, estimators, &f)
    ...
    table.rows.times do |row_i|
      ...
      predicted = f.call(CaganRegressionContext.new(row_i, table, estimators))
  ...
  def naive_numerical_deriv(x_var, row, table, estimators, &f)
    ...
    f_x_a = -f.call(CaganRegressionContext.new(row, table, estimators))
    
    estimators[x_var]  = o + h
    f_x_b = 8.0 * f.call(CaganRegressionContext.new(row, table, estimators))
    
    
    estimators[x_var]  = o - h
    f_x_c = -8.0 * f.call(CaganRegressionContext.new(row, table, estimators))
    
    estimators[x_var] = o - (2.0 * h)
    f_x_d = f.call(CaganRegressionContext.new(row, table, estimators))
    estimators[x_var] = o
